"""
Customer Support Tools for the Multilingual Agent.

Tools are exposed to the LangGraph agent via LangChain's `@tool` decorator.
The agent's LLM uses the function name, type-annotated signature, and the
docstring to decide when and how to call each tool.
"""
...
def lookup_order_status(order_id: str) -> dict[str, Any]:
    ...
    if order_id in ORDERS_DB:
        return {"success": True, "order_id": order_id, "data": ORDERS_DB[order_id]}
...
def escalate_to_human(
    issue_summary: str, customer_language: str, priority: str = "normal"
) -> dict[str, Any]:
...
"""Map of tool name -> tool object, used by agent.py to resolve the `tools:`
list in agent.yml into actual callables."""
...
if __name__ == "__main__":
    ...
    print(f"   {lookup_order_status.invoke({'order_id': 'ORD-12345'})}")

    print("\n2. Testing Return Request:")
    print(
        f"   {submit_return_request.invoke({'order_id': 'ORD-12345', 'reason': 'Product defective', 'items': ['Laptop']})}"
    )

    print("\n3. Testing Escalation:")
    print(
        f"   {escalate_to_human.invoke({'issue_summary': 'Order not received after 3 weeks', 'customer_language': 'en', 'priority': 'high'})}"
    )

    print("\n4. Testing Product Availability:")
    print(f"   {check_product_availability.invoke({'product_id': 'PROD-001'})}")

    print("\n5. Testing Order Cancellation:")
    print(f"   {cancel_order.invoke({'order_id': 'ORD-67890', 'reason': 'Changed mind'})}")
